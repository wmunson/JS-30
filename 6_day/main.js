...
function displayList(){
	...
	const html = results.map(place =>{
		...
		const sateName = place.state.replace(regex, `<span class="hl">${this.value}</span>`)
		const numbers = commas(place.population);
		return `
			<li>
				<span class="name">${cityName}, ${sateName}</span>
				<span class="population">pop: ${numbers}</span>
			</li>
		`;
	}).join('')
